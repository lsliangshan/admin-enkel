          ...
          <transition-group name="no"
            ...
            <div class="list-group-item"
              ...
              <!-- <i :class="!item.status? 'fa fa-anchor' : 'glyphicon glyphicon-pushpin'"
                 ...
                 aria-hidden="true"></i>
              {{item.enkel_entrance.title}} -->
              <div class="list-group-item-img">
                <img :src="item.enkel_entrance.cover"
                     :alt="item.enkel_entrance.title">
              </div>
              <div class="list-group-item-desc">{{item.enkel_entrance.title}}</div>
              <div class="list-group-item-header">{{index + 1}}</div>
            </div>
            <div class="list-group-item"
                 key="blank"
                 style="height: 0; box-shadow: none;"></div>
          </transition-group>
        </draggable>
...
export default {
  ...
  methods: {
    ...
    requestList () {
      return new Promise(async (resolve) => {
        let res = await this.store.dispatch('editorEntrance/list', {})
        resolve(res)
      })
    },
    requestSortedList () {
      return new Promise(async (resolve) => {
        let res = await this.store.dispatch('editorEntrance/sortedlist')
        resolve(res)
      })
    },
    requestAdd (data) {
      return new Promise(async (resolve) => {
        let res = await this.store.dispatch('editorEntrance/add', data)
        resolve(res)
      })
    },
    requestModify (data) {
      return new Promise(async (resolve) => {
        let res = await this.store.dispatch('editorEntrance/modify', data)
        resolve(res)
      })
    },
    requestDel (data) {
      return new Promise(async (resolve) => {
        let res = await this.store.dispatch('editorEntrance/del', data)
        resolve(res)
      })
    },
    requestSave (data) {
      return new Promise(async (resolve) => {
        let res = await this.store.dispatch('editorEntrance/save', data)
        resolve(res)
      })
    },
