    ...
    <div class="module_container_content">
      <Row>
        <Col v-for="(item, index) in list"
             :key="index"
             class="module_container_content_item">
        <!-- :xs="8"
             :sm="8"
             :md="6"
             :lg="4" -->
        ...
        <div class="module_container_content_item_inner">
          <div class="module_container_content_item_inner_img">
            <img :src="item.cover"
                 :alt="item.title">
          </div>
        ...
        <FormItem label="类型">
          <div style="display: flex; flex-direction: row; align-items: center;">
            <Select v-model="addModal.data.type"
                    :style="{width: (addModal.data.type == 'search' ? '320px' : '100%')}">
              <Option v-for="(item, index) in allTypes"
                      :key="index"
                      :value="item.value"
                      :label="item.label">
                <span>{{item.label}}</span>
                <span v-if="item.value == 'search'"
                      style="float:right;color:#ccc">搜索ID</span>
            ...
            <transition name="fade">
              <Input placeholder="请输入搜索内容"
                     ...
                     v-model="addModal.data.search"
                     v-if="addModal.data.type == 'search'"></Input>
            </transition>
          </div>
        </FormItem>
        <transition name="fade">
        ...
        <FormItem label="类型">
          <div style="display: flex; flex-direction: row; align-items: center;">
            <Select v-model="modifyModal.data.type"
                    :style="{width: (modifyModal.data.type == 'search' ? '320px' : '100%')}">
              <Option v-for="(item, index) in allTypes"
                      :key="index"
                      :value="item.value"
                      :label="item.label">
            ...
            <transition name="fade">
              <Input placeholder="请输入搜索内容"
                     style="margin-left: 10px;"
                     v-model="modifyModal.data.search"
                     v-if="modifyModal.data.type == 'search'"></Input>
            </transition>
          </div>
        </FormItem>
        <transition name="fade">
  ...
  methods: {
    ...
    requestList () {
      return new Promise(async (resolve) => {
        let res = await this.store.dispatch('moduleEntrance/list', {})
        resolve(res)
      })
    },
    requestAdd (data) {
      return new Promise(async (resolve) => {
        let res = await this.store.dispatch('moduleEntrance/add', data)
        resolve(res)
      })
    },
    requestModify (data) {
      return new Promise(async (resolve) => {
        let res = await this.store.dispatch('moduleEntrance/modify', data)
        resolve(res)
      })
    },
    requestDel (data) {
      return new Promise(async (resolve) => {
        let res = await this.store.dispatch('moduleEntrance/del', data)
        resolve(res)
      })
    },
    async init () {
    ...
    add () {
      ...
      this.$refs.formAdd.validate(async (valid) => {
        if (valid) {
          let requestData = JSON.parse(JSON.stringify(this.addModal.data))
